...
source("./lipidpreproc/MSDIALData_preprocess.R")
## Source will offer the following contents:
...
fasInfo <- lapply(lipids, getFAsInfo_msdial)
...
data_sub_classSum_stat3 <- lapply(data_sub_classSum_p, getPValue, "headgroup")
...
ggsave(paste0(fileLoc, "headgroup/color.pdf"), 
...
plot_black <- ggplot() +
  ...
  labs(x = "group",
       ...
       title = "Lipid class statistics") +
  theme(plot.title = element_text(hjust = 0.5, size = 20))
ggsave(paste0(fileLoc, "headgroup/black.pdf"), plot = plot_black, 
