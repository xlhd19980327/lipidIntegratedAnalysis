...
##!!!!!DEV: delOddChainOpt should always be F in the Proteins mode
dataSet <- readingLipidData(datafile = "./testData/CerebrospinalFluid_multiomics/input/proteins_Depletion_tidy.csv",
                            sampleList = "./testData/CerebrospinalFluid_multiomics/input/sampleList_lip.csv", 
                            lipField = "protein_id", controlGrp = "", dataType = "Proteins", delOddChainOpt = F,
...
if(analOpt == "group_by_group"){
  ...
  for(i in dataSet$groupsLevel[dataSet$groupsLevel != dataSet$controlGrp]){
    ...
    ##!!!!!DEV: showLipClass should always be F in the Proteins mode
